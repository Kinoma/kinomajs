...
<makefile>
	...
	<input name="$(GUITAR_SYSROOT)/usr/include"/>

	<input name="$(F_HOME)/libraries/freetype/include"/>

	<header name="FskPlatform.h"/>
	
	<source name="KplScreenLinuxGuitar.c"/>
...
-->
	...
	<c option="-DDEVICE_NAME=Guitar"/>

    <c option="--sysroot=$(GUITAR_SYSROOT)"/>
	
	<library name="-Wl,-rpath,$(GUITAR_SYSROOT)/usr/lib,-z,muldefs"/>
    <library name="-Wl,-rpath,.,-rpath,'$ORIGIN/lib'"/>
	<library name="-L$(GUITAR_SYSROOT)/usr/lib"/>
