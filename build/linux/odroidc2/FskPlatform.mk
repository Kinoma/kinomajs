...
<makefile>
	...
	<input name="$(ODROIDC2_SYSROOT)/usr/include"/>

	<input name="$(F_HOME)/libraries/freetype/include"/>

	<header name="FskPlatform.h"/>
	
	<source name="KplScreenLinuxHiKey.c"/>
...
-->
	...
	<library name="-Wl,-rpath,$(ODROIDC2_SYSROOT)/usr/lib,-z,muldefs"/>
    <library name="-Wl,-rpath,.,-rpath,'$ORIGIN/lib'"/>
	<library name="-L$(ODROIDC2_SYSROOT)/usr/lib"/>
	<library name="-L$(ODROIDC2_SYSROOT)/usr/lib/aarch64-linux-gnu"/>
	<library name="-lm"/>
	<library name="-ldl"/>
	<library name="-lrt"/>
...
-->
	...
	<c option="-DDEVICE_NAME=ODroid\ C2"/>
