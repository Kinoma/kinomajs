...
FskInstrumentedTypeRecord gKplScreenK4TypeInstrumentation = {NULL, sizeof(FskInstrumentedTypeRecord), "kplscreenpxa1088"};
#endif

//refer to linux-custom/include/video/mmp_ioctl.h
static char deviceName[] =			"/dev/fb0";

#define FBIO_WAITFORVSYNC           	_IO('m', 18)  
...
FskErr KplScreenGetBitmap(KplBitmap *bitmap)
{
	...
	if (!gKplScreen) {
		...
		if (-1 != screen->fbfd) {
			...
			gKplScreen = screen;
			gKplScreen->bitmap->width = screen->vinfo.xres_virtual;
			gKplScreen->bitmap->height = screen->vinfo.yres;
			gKplScreen->bitmap->pixelFormat = kFskBitmapFormatGLRGBA;
			gKplScreen->bitmap->depth = 32;
			gKplScreen->bitmap->rowBytes = gKplScreen->bitmap->width * (gKplScreen->bitmap->depth / 8);
		}
...
FskErr KplScreenSetUpdateInterval(void *state, void *obj, UInt32 propertyID, FskMediaPropertyValue property)
{
	...
	FskInstrumentedTypePrintfDebug(&gKplScreenK4TypeInstrumentation, "Setting update interval to %ld ms", (long)gKplScreen->updateInterval);
	...
	FskInstrumentedTypePrintfDebug(&gKplScreenK4TypeInstrumentation, 
        "Setting update interval time to %ld.%06lds", 
        (long) gKplScreen->updateIntervalTime.seconds, 
        (long) gKplScreen->updateIntervalTime.useconds);
...
static void drawingPumpUpdate(FskWindow win)
{
	...
	FskInstrumentedTypePrintfNormal(&gKplScreenK4TypeInstrumentation, "[%s] called %d ms after last update\n", threadName, FskTimeInMS(&delta));
	...
	FskInstrumentedTypePrintfNormal(&gKplScreenK4TypeInstrumentation, 
        "Next flip time %ld.%06ld \n", 
        (long) nextFlipTime.seconds,
        (long) nextFlipTime.useconds);
	...
	FskInstrumentedTypePrintfNormal(&gKplScreenK4TypeInstrumentation, "[%s] Finished callback %ld, nextFlipTime %1d.%03d\n", threadName, 
        (long) gKplScreen->callbackFiredCount, 
        (int) nextFlipTime.seconds,
        (int) nextFlipTime.useconds / 1000);
}
...
{	
	...
	if (pending > 0) {
		FskInstrumentedTypePrintfDebug(&gKplScreenK4TypeInstrumentation, "[%s] There are %d callbacks pending, skip callback %ld\n", threadName, pending, (long) callback);
		return;
	}
	...
	switch (event->type) {
			      ...
			      switch (event->code) {
				      ...
				      case ABS_MT_TRACKING_ID:
					      ...
					      if (val != -1) {
						      ...
						      cod = kFskEventNone;
						      if(sentMouseDown[currentPointer] == -1)
						      ...
						      MLOG("Track MouseUp: : currentPointer - %d, lastPointer - %d\n", currentPointer, lastPointer);
						      sentMouseDown[lastPointer] = -1;
					      }
					      break;
				      case ABS_MT_POSITION_X:
				      case ABS_X:
					      FskKplScreenPrintfDebug("code: ABS_X - touch[%d].x = %d", currentPointer, val);
					      //MLOG("code: ABS_X - touch[%d].x = %d", currentPointer, val);
		...
		case 0:
			      if (event->code == 0 && event->value == 0) {
				      if (sentMouseDown[currentPointer] == 0) {
					      FskKplScreenPrintfDebug("type, code and value all 0 - didn't send Down yet - push MouseDown\n");
					      MLOG("type, code and value all 0 - didn't send Down yet - push MouseDown - sentMouseDown[%d] = %d\n",currentPointer,sentMouseDown[currentPointer]);
					      sentMouseDown[currentPointer] = -2;		// send mousedown after we've received a location
					      cod = kFskEventMouseDown;		// too early? No x/y yet?
				      }
				      else if (currentPointer == -1)
				      {
					      MLOG("type, code and value all 0 - push NONE - sentMouseDown[%d] = %d\n",currentPointer,sentMouseDown[currentPointer]);	
					      cod = kFskEventNone;
				      }
				      else {
					      FskKplScreenPrintfDebug("type, code and value all 0 - push MouseMoved");
					      MLOG("type, code and value all 0 - push MouseMoved - sentMouseDown[%d] = %d\n",currentPointer,sentMouseDown[currentPointer]);
	...
	if (cod) {
		...
		if (cod == kFskEventMouseMoved) {
			if (sentMouseDown[currentPointer] == 0) {
				...
				MLOG(" -- mouseMoved, no eventdown sent yet. Do it. %d: %d %d\n", 
						currentPointer, touchPoints[currentPointer].x, touchPoints[currentPointer].y);
		...
		if (cod == kFskEventMouseDown) {
			...
			MLOG("Track MouseDown: : currentPointer - %d, lastPointer - %d\n", currentPointer, lastPointer);
		...
		// scale
		pt.pt.x = FskFixDiv(pt.pt.x << 16, touchScale) >> 16;
		pt.pt.y = FskFixDiv(pt.pt.y << 16, touchScale) >> 16;
		...
		MLOG("Event Submitted - code: cod = %d", cod);
		evtQueue(win, ev);
	}
