...
FskErr KplScreenGetDisplayCopy(void *state, void *obj, UInt32 propertyID, FskMediaPropertyValue property)
{
    ...
    if (gKplScreen->touchDown) {
        ...
        err = FskMemPtrNewFromData(gScreenWidth * gScreenHeight * 2, gKplScreen->baseAddr[ONSCREEN_PAGE(gKplScreen)], &bits);
        if (kFskErrNone == err) {
            FskBitmapNewWrapper(gScreenWidth, gScreenHeight, kFskBitmapFormat16RGB565LE, 16, bits, gScreenWidth * 2, (FskBitmap *)&property->value.bitmap);
            property->type = kFskMediaPropertyTypeBitmap;
        }
    }
