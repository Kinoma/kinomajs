...
static void inputHandler(struct input_event *event, inputDevice dev)
{
    ...
    val = event->value;
    ILOG("type: %d (%s) code: %d (x%x) val: %d\n", event->type, inputTypeString(event->type), event->code, event->code, event->value);  
...
//fprintf(stderr, "Keycode out of range (%x). ignore\n", event->code);
		...
		case EV_REL :
			if (0 == event->code) {				// x
				interimX = PIN(gMouseX + event->value, 0, gScreenWidth);
				ILOG("X now %d\n", interimX);
			}
			else if (1 == event->code) {		// y
				interimY = PIN(gMouseY + event->value, 0, gScreenHeight);
				ILOG("Y now %d\n", interimY);
			}
			break;
        case EV_ABS :
ILOG(" EV_ABS - ");
        switch (event->code) {
                ...
                case ABS_MT_TOUCH_MAJOR:	// doesn't happen here
					break;
				case ABS_MT_TRACKING_ID:
					ILOG("got a tracking id - assume currentPointer 1\n");
					if (val != -1) {
                        // cod = kFskEventMouseDown;
                        cod = kFskEventNone;
						currentPointer = 1;
                ...
                case ABS_X:
#if BEAGLEBONE
					val = (gScreenWidth * val) / 4000;
#endif
ILOG("ABS_X - touch[%d].x = %d\n", currentPointer, val);
					touchPoints[currentPointer].x = val;
                    break;
                case ABS_MT_POSITION_Y:
                case ABS_Y:
#if BEAGLEBONE
					val = (gScreenHeight * val) / 4000;
#endif
ILOG("code: ABS_Y - touch[%d].y = %d\n", currentPointer, val);
...
ILOG("0, 0, 0 - done with this set - curentPointer is %d\n", currentPointer);
				if (interimX || interimY) {		// these are from the mouse
					doingMouse = true;
					cod = kFskEventMouseMoved;
					if (interimX) {
						gMouseX = interimX;
						interimX = 0;			// reset for next time
					}
					if (interimY) {
						gMouseY = interimY;
						interimY = 0;
				...
				else if (sentMouseDown[currentPointer]) {
ILOG("type, code and value all 0 - push MouseMoved\n");
    ...
    if (cod) {
		...
		if (cod == kFskEventMouseMoved) {
			if (doingMouse) {
				if (sentMouseDown[currentPointer])
					addAMoved(win, gMouseX, gMouseY, ticks, currentPointer);
//				FskCursorDraw();
				goto bail;
			} else if (sentMouseDown[currentPointer]) {
...
FskErr LinuxGetCursorLocation(FskPoint location) {
	location->x = gMouseX;
	location->y = gMouseY;
	return kFskErrNone;
}

FskErr LinuxSetCursorLocation(FskPoint location) {
	FskWindow win = FskWindowGetActive();
	if (!win->usingGL)
		FskFrameBufferHideCursor(NULL);
	gMouseX = location->x;
	gMouseY = location->y;
	if (!win->usingGL)
		FskFrameBufferShowCursor(NULL);
}
...
void initializeLinuxInput(int width, int height)
{
	...
	fbv->doGetCursorLocation = LinuxGetCursorLocation;
	fbv->doSetCursorLocation = LinuxSetCursorLocation;
