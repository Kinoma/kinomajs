...
kcl_int_str2ix(kcl_int_t *ai, char *s)
{
	...
	bn = ai->data;
	memset(bn->data, 0, bn->size * sizeof(bn_word));
	n = 0;
	for (p = s + len; --p >= s;) {
		c = *p;
		if (c >= '0' && c <= '9')
...
bn_allocbuf(bn_context_t *ctx, unsigned int n)
{
	...
	if (ctx->bn_bp + n > ctx->bn_bend) {
		...
		if (ctx->bn_cbuf->next == NULL) {
			if ((ctx->bn_cbuf->next = kcl_malloc(sizeof(struct bn_buf))) == NULL)
				bn_throw(ctx, BN_ERR_NOMEM);
			ctx->bn_cbuf = ctx->bn_cbuf->next;
			ctx->bn_cbuf->next = NULL;
		}
		else
			ctx->bn_cbuf = ctx->bn_cbuf->next;
		ctx->bn_bp = ctx->bn_cbuf->buf;
...
bn_freebuf(bn_context_t *ctx, void *ptr)
{
	...
	if (p >= ctx->bn_cbuf->buf && p < ctx->bn_bend) {
		ctx->bn_bp = p;
		return;
	}
	/* search the block */
	for (bp = ctx->bn_buflist; bp != NULL; bp = bp->next) {
		if (p >= bp->buf && p < bp->buf + BN_BUFSIZE) {
			ctx->bn_cbuf = bp;
			ctx->bn_bp = p;
			return;
		}
...
kcl_z_alloc(kcl_z_t **rp)
{
	if ((*rp = kcl_malloc(sizeof(bn_context_t))) == NULL)
		return KCL_ERR_NOMEM;
	memset(*rp, 0, sizeof(bn_context_t));
	return KCL_ERR_NONE;
}

kcl_err_t
kcl_z_init(kcl_z_t *z, kcl_error_callback_t *cb)
{
	bn_context_t *ctx = z;

	if ((ctx->bn_buflist = kcl_malloc(sizeof(struct bn_buf))) == NULL)
		return KCL_ERR_NOMEM;
...
kcl_z_free(kcl_z_t *z)
{
	if (z != NULL) {
		bn_context_t *ctx = z;
		struct bn_buf *bp, *next;
		for (bp = ctx->bn_buflist; bp != NULL; bp = next) {
			next = bp->next;
			kcl_free(bp);
		}
