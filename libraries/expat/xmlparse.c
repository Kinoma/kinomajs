...
#ifdef __FSK_LAYER__
#include "FskMemory.h"
static void *XML_FskMalloc(size_t s) {return FskMemPtrAlloc(s);}
static void *XML_FskRealloc(void *p, size_t s) {return FskMemPtrReallocC(p, s);}
static void XML_FreFree(void *p) {FskMemPtrDispose(p);}
static XML_Memory_Handling_Suite FskMemSuite = {XML_FskMalloc, XML_FskRealloc, XML_FreFree};
#endif
...
{
#ifdef __FSK_LAYER__
  return XML_ParserCreate_MM(encodingName, &FskMemSuite, NULL);
#else
...
XML_ParserCreateNS(const XML_Char *encodingName, XML_Char nsSep)
{
  ...
  *tmp = nsSep;
#ifdef __FSK_LAYER__
  return XML_ParserCreate_MM(encodingName, &FskMemSuite, tmp);
#else
