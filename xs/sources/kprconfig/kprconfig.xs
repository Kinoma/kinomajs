...
//			aFile.write('\tFskTextEngine fte;\n');
		...
		<function name="generateCMake" params="xscIncludePaths, xscPackages, makefiles, config, permissions, entitlements">
			...
			for (i = 0; i < c; i++) {
				var item = this.xmlPaths[i];
				var targetName = item.destinationPath.replace(xsTool.slash, '_');
			...
			if (c > 0) {
				for (i = 0; i < c; i++)
					aFile.write('link_directories(' + linkIncs[i] + ')\n');
				aFile.write('\n');
			}
			...
			if (c > 0) {
				for (var i = 0; i < c; i++)
					aFile.write('list(APPEND CMAKE_PREFIX_PATH ' + frameworkPaths[i] + ')\n');
				aFile.write("\n");
			}
		...
		<function name="generateFskPlatform">
			...
			if (this.platform == "iphone")
				srcPath = this.home + "/build/iphone/FskPlatform.iPhone.h";
			else if (this.platform == "mac")
				srcPath = this.home + "/build/mac/FskPlatform.Mac.h";
			else if (this.platform == "win")
				srcPath = this.home + "/build/windows/FskPlatform.Win.h";
			else if (this.subplatform)
				srcPath = this.home + "/kinoma/kpr/make/" + this.platform + "/" + this.subplatform + "/FskPlatform.h";
			else
