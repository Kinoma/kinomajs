...
void
KPDHTrivialClipPolygon(register int nPts, register KPDVector3D *p, int *inCode, int *outCode)
{
	register int code, in, out;

	for (in = KPDHCLIP_ALLIN, out = KPDHCLIP_ALLOUT; nPts--; p++) {
		code = SpaceCode(p);
		in  |= code;
		out &= code;
	}
...
static void
ClipPolygonAgainstPlane(int planeID, int *nPts, KPDVector3D *pts0, KPDVector3D *pts1)
{
	...
	for (k = 0, nOut = 0, n1 = 0, d = 0, d0 = 0; k <= numPts; k++, d0 = d, n1 = n0) {
		n0 = (k == numPts) ? 0 : k;
		switch (planeID) {
			case KPDHCLIP_LEFT:		d = pts0[n0].z + pts0[n0].x;	break;
			case KPDHCLIP_RIGHT:	d = pts0[n0].z - pts0[n0].x;	break;
			case KPDHCLIP_TOP:		d = pts0[n0].z + pts0[n0].y;	break;
			case KPDHCLIP_BOTTOM:	d = pts0[n0].z - pts0[n0].y;	break;
		}
...
static int
ClipPolygonAgainstPlanes(int *nPts, KPDVector3D *pts0, KPDVector3D *pts1, int in)
{
	int clipTotal = 0;
	int planeID;

	for (planeID = 0; planeID < KPDHCLIP_PLANES; planeID++) {
		if (in & (1 << planeID)) {
...
void
KPDHClipPolygon(int *nPts, KPDVector3D *pts, KPDVector3D *temp, const KPRect *r)
{
	int			inCode, outCode, i;
	double		x0, y0, dx, dy, idx, idy;
	register	KPDVector3D *p;
	...
	KPDHTrivialClipPolygon(*nPts, pts, &inCode, &outCode);

	if (outCode == 0) {
		if (inCode != KPDHCLIP_ALLIN) {	/* Need to clip */
			int clipCode;
