	...
	<object name="kprGrammar">
		...
		<object name="skin" prototype="kprGrammar.item" pattern="skin">
			...
			<function name="printConstructor" params="out">
				out.cat("new Skin(");
				if (this.texture) {
					out.cat(this.texture, ", ");
					out.cat("{x:", this.x, " , y:", this.y, " , width:", this.width, " , height:", this.height, "}, ", this.variants.offset, ", ", this.states.offset, ", ");
					this.printCoordinates(out, this.tiles, kprGrammar.tiles)
					out.cat(", ");
					this.printCoordinates(out, this.margins, kprGrammar.margins);
					out.cat(", '", this.aspect, "'");
				}
				else {
					if (this.color)
						...
						out.cat("null");
					out.cat(", ");
					this.printCoordinates(out, this.borders, kprGrammar.borders)
					out.cat(", ");
					if (this.borders.color)
						out.cat("'", this.borders.color, "'");
			...
			<function name="printCoordinates" params="out, instance, prototype">
		...
		<object name="style" prototype="kprGrammar.item" pattern="style">
			...
			<function name="printConstructor" params="out">
				...
				out.cat(", ");
				if (this.horizontal)
					out.cat("'", this.horizontal, "'");
				else
					out.cat("null");
				out.cat(", ");
				out.cat(this.margins.left, ", ");
				out.cat(this.margins.right, ", ");
				out.cat(this.indentation, ", ");
				if (this.vertical)
					out.cat("'", this.vertical, "'");
				else
					out.cat("null");
				out.cat(", ");
				out.cat(this.margins.top, ", ");
				out.cat(this.margins.bottom, ", ");
				out.cat(this.leading, ", ");
				out.cat(this.lines);
				out.cat(");");
			</function>
		</object>
		...
		<object name="behavior" prototype="kprGrammar.item" pattern="behavior">
			...
			<function name="print" params="out">
				...
				out.cat("function() {");
				out.tab(1);
				out.line("this.onCreate.apply(this, arguments);");
				out.tab(-1);
				out.line("}");
				out.at(this.__xs__line);
			...
			<function name="printScript" params="out">
				...
				text = text.replace(this.top, "$1$2: { value: function");
		...
		<object name="handler" prototype="kprGrammar.item" pattern="handler">
			...
			<function name="print" params="out">
				out.line("var $ = new Handler(\"", this.path, "\");");
				if (this.behaviorID) {
					out.line("$.behavior = new ", this.behaviorID, "();");
				}
				else if (this.behavior != kprGrammar.behavior) {
					out.at(this.behavior.__xs__line);
					out.line("$.behavior");
					this.behavior.printObject(out);
				}
				out.line("Handler.put($);");
			</function>
		</object>
		...
		<object name="iterate" prototype="kprGrammar.any" pattern="iterate">
			...
			<function name="printContent" params="out">
				...
				out.tab(1);
				out.scope();
				out.line(this.on, ".forEach(function($) {");
				out.tab(1);
		...
		<object name="content" prototype="kprGrammar.any" pattern="content">
			...
			<string name="keys" value="_" pattern="@keys"/>
			...
			<function name="prepareContent" params="out">
				...
				if (this.behavior != kprGrammar.behavior) {
					if (this.behaviorID)
						out.reportError(this, "a content element cannot have a behavior attribute and a behavior element");
					else if (this.behavior.id)
						out.reportError(this.behavior, "nested behavior element cannot have an id attribute");
					else {
						// do not nest behavior objects to avoid closures
						var template = out.current;
						var behaviors = template.behaviors;
						this.behavior.id = this.behaviorID = template.id + ".behaviors[" + behaviors.length + "]";
						this.behavior.embed = true;
			...
			<function name="print" params="out">
				...
				out.at(this.__xs__line);
				out.printVar(this.id);
				out.cat("function($, _, coordinates) {");
				out.tab(1);
				var coordinates = out.serializeCoordinates(this);
				out.line("var ", out.temporary, " = Content.combineCoordinates(", coordinates, ", coordinates);");
				if (this.like) {
					out.line(this.like, ".call(this, ", this._with, ", ", this.keys, ", ", out.temporary, "); ");
					if (this.behaviorID)
						out.cat(out.temporary, " = ", this.behaviorID, ";");
					else
						out.cat(out.temporary, " = ", out.temporary, ".id;");
				}
				else {
					out.line(this.constructor, ".call(this, ", out.temporary, "); ");
					if (this.behaviorID)
						out.cat(out.temporary, " = ", this.behaviorID, ";");
					else
						out.cat(out.temporary, " = undefined;");
				}
				this.printProperties(out);
				out.line("if (coordinates) coordinates.id = ", out.temporary, ";");
				out.line("else if (", out.temporary, ") ", out.content, ".behavior = new ", out.temporary, "(", out.content, ", $, _);");
				this.printURL(out);
				out.tab(-1);
				out.line("};");
				out.line(this.id, ".prototype = ", this.constructor, ".prototype;");
			...
			<function name="printConstructor" params="out">
				out.at(this.__xs__line);
				var coordinates = out.serializeCoordinates(this);
				out.line("var ", out.temporary, " = ", coordinates, "; ");
				out.cat("var ", out.content);
				if (this.anchor) {
					if (this.isExpression(this.anchor))
						out.cat(" = $[", this.anchor, "]");
					else
						out.cat(" = $.", this.anchor);
				}
				if (this.like) {
					out.cat(" = new ", this.like, "(", this._with, ", ", this.keys, ", ", out.temporary, "); ");
					if (this.behaviorID)
						out.cat(out.temporary, " = ", this.behaviorID, ";");
					else
						out.cat(out.temporary, " = ", out.temporary, ".id;");
					...
					out.cat(" = new ", this.constructor, "(", out.temporary, "); ");
					if (this.behaviorID)
						out.cat(out.temporary, " = ", this.behaviorID, ";");
					else
						out.cat(out.temporary, " = undefined;");
			...
			<function name="printContent" params="out">
				this.printConstructor(out);
				this.printProperties(out);
				out.line("if (", out.temporary, ") ", out.content, ".behavior = new ", out.temporary, "(", out.content, ", ", this._with, ", ", this.keys, ");");
				this.printURL(out);
				out.line(out.container, ".add(", out.content, ");");
			</function>
			<function name="printProperties" params="out">
				out.at(this.__xs__line);
				out.line();
				if (this.skinID)
					out.cat(out.content, ".skin = ", this.skinID, "; ");
				else if (this.skin != kprGrammar.skin) {
					out.cat(out.content, ".skin = ");
					this.skin.printConstructor(out);
					out.cat("; ");
				}
				if (this.styleID)
					out.cat(out.content, ".style = ", this.styleID, "; ");
				else if (this.style != kprGrammar.style) {
					out.line(out.content, ".style = ");
				...
				if (this.hasOwnProperty("name")) {
					if (this.isExpression(this.name))
						out.cat(out.content, ".name = ", this.name, "; ");
					else
						out.cat(out.content, ".name = '", this.name, "'; ");
				}
			</function>
			<function name="printProperty" params="out, property">
				if (this.hasOwnProperty(property)) {
					out.cat(out.content, ".", property, " = ", this[property], "; ");
				}
			</function>
			<function name="printURL" params="out">
			</function>
		</object>
		...
		<object name="scroller" prototype="kprGrammar.container" pattern="scroller">
			...
			<function name="printProperties" params="out">
				...
				if (this.loop)
					out.cat(out.content, ".loop = ", this.loop, "; ");
			</function>
		</object>
		...
		<object name="label" prototype="kprGrammar.content" pattern="label">
			...
			<function name="printProperties" params="out">
				...
				if (this.string)
					out.cat(out.content, ".string = ", this.string, "; ");
			</function>
		</object>
		
		<object name="span" pattern="span">
			<string name="string" pattern="@string"/>
			<string name="style" value="null" pattern="@style"/>
			<function name="prepareContent" params="out">
			</function>
			<function name="print" params="out">
				out.at(this.__xs__line);
				if (this.string) {
					out.line(out.container, ".beginSpan(", this.style, "); ");
					out.cat(out.container, ".concat(", this.string, ");");
					out.cat(out.container, ".endSpan();");
				}
			</function>
		</object>
		...
		<object name="wrap" pattern="wrap">
			...
			<function name="print" params="out">
				...
				this.content.printProperties(out);
				out.line(out.container, ".concat(", out.content, ", '", this.alignment, "');");
			</function>
		</object>
		...
		<object name="block" pattern="block">
			...
			<function name="print" params="out">
				...
				if (this.spans.length) {
					out.line(out.container, ".beginBlock(", this.style, ");");
					this.spans.forEach(this.printSpan, out); 	
					out.line(out.container, ".endBlock();");
				}
				else if (this.string) {
					out.line(out.container, ".beginBlock(", this.style, ");");
					out.cat(out.container, ".concat(", this.string, ");");
		...
		<object name="text" prototype="kprGrammar.content" pattern="text">
			...
			<function name="printProperties" params="out">
				...
				if (this.blocks.length) {
					out.line(out.content, ".begin();");
					out.nest(1);
					this.blocks.forEach(this.printBlock, out); 	
					out.nest(-1);
					out.line(out.content, ".end();");
				}
				else if (this.string)
					out.cat(out.content, ".string = ", this.string, "; ");
			</function>
		...
		<object name="resource" prototype="kprGrammar.content">
			...
			<function name="printProperties" params="out">
				...
				if (this.aspect) {
					...
					case "fit":
					case "stretch":
						out.cat(out.content, ".aspect = '", this.aspect, "'; ");
						break;	
					default:
						out.cat(out.content, ".aspect = ", this.aspect, "; ");
						break;	
			...
			<function name="printURL" params="out">
				if (this.hasOwnProperty("url")) {
					if (this.hasOwnProperty("mime"))
						out.line(out.content, ".load(", this.url, ", ", this.mime, "); ");
					else
						out.line(out.content, ".load(", this.url, "); ");
				}
			</function>
		</object>
		...
		<object name="program" pattern="/program">
			...
			<function name="printExport" params="id">
			</function>
			...
			<function name="reportError" params="instance, message">
				if (instance)
					trace(instance.__xs__path + ":" + instance.__xs__line + ": ");
//				trace(message + "\n");
//				debugger;
