...
static void KprDebugMachineCallbackText(KprDebugMachine machine, char* function, int view, char* text, char* value);
...
static void KprDebugMachineChangeTitle(KprDebugMachine self, char* theTitle, char* theValue);
...
FskErr KprDebugMachineNew(KprDebugMachine* it, KprDebug debug, FskSocket skt)
{
	...
	KprDebugMachineCallbackText(self, "onMachineRegistered", mxNoCommand, NULL, NULL);
...
void KprDebugMachineCallbackText(KprDebugMachine machine, char* function, int view, char* text, char* value)
{
	KprDebug self = machine->debug;
	xsMachine* the = self->the;
	if (xsTypeOf(self->behavior) == xsUndefinedType) goto bail;
		...
		xsTry {
			...
			if (xsFindResult(xsVar(0), xsID(function))) {
				if (view >= 0)
					...
					(void)xsCallFunction3(xsResult, xsVar(0), xsString(machine->address), xsString(text), xsString(value));
				else if (text)
...
{	
	if (self->view == mxLogView) {
		if (self->path[0] && (self->line >= 0)) {
			KprDebugMachinePrintView(self, mxLogView, "%s:%d: %s", self->path, self->line, theData);
			self->path[0] = 0;
			self->line = -1;
		}
		else if (FskStrLen(theData))
...
void KprDebugMachineParseTag(KprDebugMachine self, char* theName)
{
	if (strcmp(theName, "break") == 0) {
		...
		self->broken = 0;
		self->logging = 1;
		KprDebugMachineChangeTitle(self, self->name, self->value);
	}
...
void KprDebugMachineChangeTitle(KprDebugMachine self, char* theTitle, char* theValue)
{
	FskErr err = kFskErrNone;
	
	// FskDebugStr("%s: %s", __FUNCTION__, theTitle);
	BAIL_IF_NULL(self->title = FskStrDoCopy(theTitle), err, kFskErrMemFull);
	KprDebugMachineCallbackText(self, "onMachineTitleChanged", mxNoCommand, theTitle, theValue);
bail:
	return;
}
	...
	if (xsTypeOf(debug->behavior) == xsUndefinedType) goto bail;
	{
		xsTry {
			...
			if (flags == ' ')
				...
				xsNewHostProperty(xsVar(2), xsID("state"), xsInteger(3), xsDefault, xsDontScript);
			else
				xsNewHostProperty(xsVar(2), xsID("state"), xsInteger(0), xsDefault, xsDontScript);
			if (path)
				xsNewHostProperty(xsVar(2), xsID("path"), xsString(path), xsDefault, xsDontScript);
...
//	FskDebugStr("%s: %d - %s:%d", __FUNCTION__, theView, thePath, theLine);
	...
	if (xsFindResult(xsVar(0), xsID("onMachineFileChanged"))) {
		(void)xsCallFunction3(xsResult, xsVar(0), xsString(self->address), thePath ? xsString(thePath) : xsNull, xsInteger(theLine));
	}
...
void KprDebugMachinePrintView(KprDebugMachine self, int theView, char* theFormat, ...)
{
	...
	KprDebugMachineCallbackText(self, "onMachineViewPrint", theView, aBuffer, NULL);
}
...
void KPR_debug_addBreakpoints(xsMachine *the)
{
	KprDebug self = xsGetHostData(xsThis);
	KprDebugMachine machine = NULL;
	...
	char* path = NULL;
	SInt32 line = -1;
	if ((c > 1) && (xsTypeOf(xsArg(0)) == xsStringType) && (xsIsInstanceOf(xsArg(1), xsArrayPrototype))) {
		char* address = xsToString(xsArg(0));
		machine = KprDebugFindMachine(self, address);
		strcpy(aBuffer, "HTTP/1.1 200 OK\15\12Content-type: xml/xsbug\15\12\15\12");
		strcat(aBuffer, "<?xml version=\"1.0\" encoding=\"UTF-8\"?>");
		...
		if ((c > 2) && (xsTest(xsArg(2))))
			strcat(aBuffer, "<breakpoint path=\"start\" line=\"0\"/>");
		if ((c > 3) && (xsTest(xsArg(3))))
			strcat(aBuffer, "<breakpoint path=\"exceptions\" line=\"0\"/>");
...
void KPR_debug_logout(xsMachine *the)
{
	KPR_debug_trigger(the, mxLogoutCommand);
}
