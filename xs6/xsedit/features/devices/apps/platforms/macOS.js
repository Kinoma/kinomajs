...
class MacOSTileBehavior extends TileBehavior {
}

class MacOSViewBehavior extends Behavior {
	onCreate(container, data) {
		...
		data.PREFERENCES.insert(new MacOSPreferences(preferences), data.PREFERENCES.first);
	}
...
export const MacOSTile = Container.template($ => ({
	left:0, top:0, height:105, skin:greenTileSkin, style:tileStyle,
	Behavior: MacOSTileBehavior,
	contents: [
...
export const MacOSView = Container.template($ => ({
	left:0, right:10, top:0, bottom:10, skin:bodySkin,
	Behavior: MacOSViewBehavior,
	contents: [
		Container($, {
...
const MacOSPreferences = Column.template($ => ({
