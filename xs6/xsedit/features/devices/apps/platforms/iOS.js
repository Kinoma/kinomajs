...
class iOSTileBehavior extends TileBehavior {
}

class iOSViewBehavior extends Behavior {
	onCreate(container, data) {
		...
		data.PREFERENCES.insert(new iOSPreferences(preferences), data.PREFERENCES.first);
	}
...
export const iOSTile = Container.template($ => ({
	left:0, top:0, height:105, skin:greenTileSkin, style:tileStyle,
	Behavior: iOSTileBehavior,
	contents: [
...
export const iOSView = Container.template($ => ({
	left:0, right:10, top:0, bottom:10, skin:bodySkin,
	Behavior: iOSViewBehavior,
	contents: [
		Container($, {
...
const iOSPreferences = Column.template($ => ({
