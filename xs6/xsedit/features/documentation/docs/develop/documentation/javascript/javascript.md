...
<!--From CR re the following: Currently no event returns a value. If that changes, a "Returns:" line will be added for events in the template for creating reference subsections, and the following will be restored here: "If a return value is indicated, it is required unless noted otherwise."-->
...
<!--From CR: Has "xkpr" changed now that the term "KPR" is no longer being used?-->
...
<!--From Peter re the following: Current API is onInvoke. Need to add alias to make this work. CR replies: done, so delete this comment?-->
...
<!--From CR re the following: Is it correct that I deleted the "opacity" parameter? If not, add and explain below. (If correct, delete this comment.)-->
...
<!--From CR re description below: Is the default 0? If not, add "The default value is 1." (If so, delete this comment.)-->
...
<!--From Peter re the following: Where do we define system codes?-->
...
<!--From Peter: What is the second paragraph of the following description trying to tell me?-->
...
<!--From CR: Please provide other parameters below, with descriptions.-->
