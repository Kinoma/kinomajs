...
export class CodeBehavior extends Behavior {
	...
	doCopy(code) {
		shell.behavior.setClipboard(code.selectionString);
	}
	doCut(code) {
		this.doCopy(code);
		this.doClear(code);
	...
	onMouseEntered(code, x, y) {
		shell.behavior.cursorShape = system.cursors.iBeam;
	}
	onReveal(code) {
...
export class LineBehavior extends Behavior {
	changeState(line) {
		switch(this.flags) {
		case 0: line.state = 0; break;
		case 1: line.state = 1; break;
		case 2: case 3: line.state = 2; break;
		default: line.state = 3; break;
		}
	}
...
export class HeaderBehavior extends LineBehavior {
	changeArrowState(line, state) {
		line.first.next.state = state;
	}
	expand(line, expandIt) {
		this.changeArrowState(line, expandIt ? 3 : 1);
	}
	onCreate(line, data) {
