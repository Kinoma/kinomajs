...
var pk = new Crypt.PKCS1_5(Crypt.PKCS8.parse(Files.readChunk("xssig.priv")), true);
...
for (var argi = 1, argc = argv.length; argi < argc; argi++) {
	...
	var h = (new Crypt.SHA1()).process(Files.readChunk(xsb));
	console.log("h = " + (new Arith.Integer(h)).toString(16));
	Files.writeChunk(sig, pk.sign(h));
}
