...
C_OPTIONS = -I$(XS6_MC_DIR) -I$(XS6_INC_DIR) -I$(XS6_SRC_DIR) -I$(XS6_SRC_DIR)/pcre -I$(XS6_SRC_DIR)/mc -I$(XS6_MC_DIR)/extensions/crypt -DmxRun=1 -DmxMC=1 -DWMSDK_VERSION=$(WMSDK_VERSION) -DXIP=$(XIP) -DXS_ARCHIVE=$(XS_ARCHIVE)
...
.PHONY: modules extensions archive ftfs
.SUFFIXES: .config .temp .update

all: $(TMP_DIR) $(BIN_DIR) $(DEST_DIR) $(DEST_DIR)/application.xsb $(DEST_DIR)/inetd.xsb $(DEST_DIR)/launcher.xsb $(BIN_DIR)/libext.a $(BIN_DIR)/libstubs.a modules extensions $(BIN_DIR)/$(PROGRAM)_$(BOARD).axf $(BIN_DIR)/$(PROGRAM)_$(BOARD).bin ftfs $(BIN_DIR)/flash_$(BOARD).config $(BIN_DIR)/layout.txt $(BIN_DIR)/mw30x_uapsta.bin $(BIN_DIR)/boot2.bin
...
modules:
	(cd modules; for i in *.make; do make -I $(XS6_MC_DIR) -f $$i $(mod_target); done)

extensions:
	(cd extensions; for i in *.make; do make -I $(XS6_MC_DIR) -f $$i $(mod_target); done)
...
clean:
	rm -f $(OBJECTS) $(GEN_FILES) $(TMP_DIR)/ext_stubs.sym.h $(TMP_DIR)/ext_stubs.o $(TMP_DIR)/$(LIBMODULE) $(TMP_DIR)/stubs/*.[so] $(addsuffix .d, $(basename $(OBJECTS))) $(TMP_DIR)/application.xsb $(TMP_DIR)/inetd.xsb $(TMP_DIR)/launcher.xsb
