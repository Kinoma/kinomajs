...
<package delete="false" enum="false" set="false" script="true">
		...
		Grammar.RuleSet = class {
			...
			dump() {
				...
				this.defaultRules.forEach(rule => { 
					console.log("\t", rule.io, rule.names)
				});
				...
				for (var namespace in this.elementRules) {
					...
					for (var name in nameRules) {
						...
						if (rule.grammar && !rule.grammar.dumped) {
							console.log("####", name);
							rule.grammar.dump(); 
						}
					}
			...
			dumpRules(namespaceRules) {
				for (var namespace in namespaceRules) {
					...
					for (var name in nameRules) {
						var rule = nameRules[name];
						console.log("\t", name, rule.io, rule.names)
					}
				}
			}
		}
	...
	<patch prototype="xs">
		...
		<number name="PARSE_SNIFF" value="8"/>
		<number name="PARSE_DEBUG" value="128"/>
		<function name="parse" c="xs_parse"/>
		<function name="serialize" c="xs_serialize"/>
	</patch>
