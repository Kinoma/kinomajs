...
#if BG3CDP
void (*g_ampvp_draw_vector)(void *hBD)=NULL;
void (*g_ampvp_dispose_vector)(void *hBD)=NULL;
void (*g_ampvp_set_display_vector)(int s_x, int s_y, int s_w, int s_h, int d_x, int d_y, int d_w, int d_h, int flip_mode)=NULL;
...
{
	//fprintf( stderr, "$$$ into AmpVoutDraw\n" );
    if( srcBM->hBD  != NULL )
    {
		int s_x = srcRect->x;
		int s_y = srcRect->y;
		int s_w = srcRect->width;
		int s_h = srcRect->height;
		int d_x = dstRect->x;
		int d_y = dstRect->y;
		int d_w = dstRect->width;
		int d_h = dstRect->height;
		int flip_mode = 0;

		//ampvp_set_display(s_x, s_y, s_w, s_h, d_x, d_y, d_w, d_h, flip_mode );

		if( (*g_ampvp_draw_vector) != NULL )
			(*g_ampvp_draw_vector)( srcBM->hBD );
    }
    //else
	//	fprintf( stderr, "$$$ no srcBM->hBD attached!!!\n" );

    return kFskErrNone;
}

void ampBitmapDraw(FskPort port, FskBitmap bits, FskConstRectangle srcRect, FskConstRectangle dstRect)
{
       ...
       if (FskPortAccumulateChange(port, &r))
       {
               ...
               AmpVoutDraw(bits, srcRect, dstRect);
       }
}

FskPort gAmpVpPort = NULL;
struct FskPortVectorsRecord gAmpRenderPort = {
    NULL,
    NULL,
    NULL,
    NULL,
