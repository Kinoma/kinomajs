...
/* BAIL macros */
    ...
    #define BAIL(code)							do { err = (code);		  FskInstrumentedTypePrintfForLevel(FskInstrumentationGetErrorInstrumentation(), kFskInstrumentationLevelNormal, "BAIL %s (%d) in function %s in file %s at line %d", FskInstrumentationGetErrorString((code)), (int)(code), __FUNCTION__, __FILE__, __LINE__); goto bail; } while(0)
    #define BAIL_IF_ERR(err)					do { FskErr scratchErr = (err);  if ((scratchErr) != 0)	{ FskInstrumentedTypePrintfForLevel(FskInstrumentationGetErrorInstrumentation(), kFskInstrumentationLevelNormal, "BAIL_IF_ERROR %s (%d) in function %s in file %s at line %d", FskInstrumentationGetErrorString((scratchErr)), (int)(scratchErr), __FUNCTION__, __FILE__, __LINE__); goto bail; } } while(0)
    #define BAIL_IF_NULL(q, err, code)			do { if ((q) == NULL)	{ FskInstrumentedTypePrintfForLevel(FskInstrumentationGetErrorInstrumentation(), kFskInstrumentationLevelDebug, "BAIL_IF_NULL %s (%d) in function %s in file %s at line %d", FskInstrumentationGetErrorString((code)), (int)(code), __FUNCTION__, __FILE__, __LINE__); (err) = (code); goto bail; } } while(0)
    #define BAIL_IF_ZERO(q, err, code)			do { if ((q) == 0)		{ FskInstrumentedTypePrintfForLevel(FskInstrumentationGetErrorInstrumentation(), kFskInstrumentationLevelDebug, "BAIL_IF_ZERO %s (%d) in function %s in file %s at line %d", FskInstrumentationGetErrorString((code)), (int)(code), __FUNCTION__, __FILE__, __LINE__); (err) = (code); goto bail; } } while(0)
    #define BAIL_IF_NONZERO(q, err, code)		do { if ((q) != 0)		{ FskInstrumentedTypePrintfForLevel(FskInstrumentationGetErrorInstrumentation(), kFskInstrumentationLevelDebug, "BAIL_IF_NONZERO %s (%d) in function %s in file %s at line %d", FskInstrumentationGetErrorString((code)), (int)(code), __FUNCTION__, __FILE__, __LINE__); (err) = (code); goto bail; } } while(0)
    #define BAIL_IF_NEGATIVE(q, err, code)		do { if ((q) <  0)		{ FskInstrumentedTypePrintfForLevel(FskInstrumentationGetErrorInstrumentation(), kFskInstrumentationLevelDebug, "BAIL_IF_NEGATIVE %s (%d) in function %s in file %s at line %d", FskInstrumentationGetErrorString((code)), (int)(code), __FUNCTION__, __FILE__, __LINE__); (err) = (code); goto bail; } } while(0)
    #define BAIL_IF_NONPOSITIVE(q, err, code)	do { if ((q) <= 0)		{ FskInstrumentedTypePrintfForLevel(FskInstrumentationGetErrorInstrumentation(), kFskInstrumentationLevelDebug, "BAIL_IF_NONPOSITIVE %s (%d) in function %s in file %s at line %d", FskInstrumentationGetErrorString((code)), (int)(code), __FUNCTION__, __FILE__, __LINE__); (err) = (code); goto bail; } } while(0)
    #define BAIL_IF_TRUE(q, err, code)			do { if (q)				{ FskInstrumentedTypePrintfForLevel(FskInstrumentationGetErrorInstrumentation(), kFskInstrumentationLevelDebug, "BAIL_IF_TRUE %s (%d) in function %s in file %s at line %d", FskInstrumentationGetErrorString((code)), (int)(code), __FUNCTION__, __FILE__, __LINE__); (err) = (code); goto bail; } } while(0)
    #define BAIL_IF_FALSE(q, err, code)			do { if (!(q))			{ FskInstrumentedTypePrintfForLevel(FskInstrumentationGetErrorInstrumentation(), kFskInstrumentationLevelDebug, "BAIL_IF_FALSE %s (%d) in function %s in file %s at line %d", FskInstrumentationGetErrorString((code)), (int)(code), __FUNCTION__, __FILE__, __LINE__); (err) = (code); goto bail; } } while(0)
#else
