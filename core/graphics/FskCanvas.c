 ...
 * This is actually a modification of Alex's original code, by Rich Geldreich <richgel99@gmail.com>, last updated Oct. 13, 2013,
 * so PNG files generated by this function pass pngcheck.
 *
 ...
 * in the software to the public domain. We make this dedication for the benefit of the public at large and to the detriment
 * of our heirs and successors. We intend this dedication to be an overt act of relinquishment in perpetuity of all present
...
static FskErr PNGEncode(const void *baseAddr, SInt32 width, SInt32 height, SInt32 pixBytes, SInt32 rowBytes, void **pngData, UInt32 *pngSize) {
	...
	*pngSize = 0;
	...
	for (h = height; h--; row += rowBytes) {														/* Compress image */
...
/****************************************************************************//**
 * Encode a bitmap as a PNG.
 *	\param[in]	bm			the image to be written.
 *	\param[in]	fileName	the path to where the image file is to be written.
 *	\return		kFskErrNone	if the images was compressed and written successfully.
 ********************************************************************************/

static FskErr FskBitmapEncodePNG(FskConstBitmap bm, void **pngData, UInt32 *pngSize) {
	FskErr	err;
	UInt32	pixelFormat;
	FskCanvas2dImageDataRecord	id;

	*pngData = NULL;
	*pngSize = 0;
	BAIL_IF_ERR(err = FskBitmapReadBegin((FskBitmap)bm, NULL, NULL, NULL));
	id.width = bm->bounds.width;
	id.data.length = bm->rowBytes * (id.height = bm->bounds.height);
	id.data.bytes = bm->bits;
	switch (pixelFormat = bm->pixelFormat) {
		case kFskBitmapFormat32ABGR: case kFskBitmapFormat32ARGB: case kFskBitmapFormat32BGRA:
			(void)FskCanvasConvertImageDataFormat(&id, pixelFormat, kFskBitmapFormat32RGBA);
...
FskBitmapToDataURL(FskConstBitmap bm, const char *type, float quality, char **dataURL)
{
	...
	UInt32				cmprSize, dur, cto, frt, b64Size, headerSize, urlSize, enc;
	char				*url;
	...
	*dataURL = NULL;
	...
	if      (!FskStrCompare(type, "image/jpeg"   ))	enc = 'j';
	else if (!FskStrCompare(type, "image/png"    ))	enc = 'p';
	else if (!FskStrCompare(type, "image/bmp"    ))	enc = 'b';
	else if (!FskStrCompare(type, "image/svg+xml"))	enc = 0;
	else											enc = 0;

	/* Compress image */
	if (enc) do {														/* First try our compressor */
		if (kFskErrNone != (err = FskImageCompressNew(&ic, bm->pixelFormat, type, bm->bounds.width, bm->bounds.height))) break;
		if ('j' == enc && quality > 0) {
			mpvr.type = kFskMediaPropertyQuality;
			mpvr.value.number = quality;
			if (kFskErrNone != (err = FskImageCompressSetProperty(ic, kFskMediaPropertyQuality, &mpvr))) break;
		}
		if (kFskErrNone != (err = FskImageCompressFrame(ic, (FskBitmap)bm, &cmprImg, &cmprSize, &dur, &cto, &frt, NULL, NULL))) break;
		if (kFskErrNone != (err = FskImageCompressFlush(ic))) break;
	} while(0);
	FskImageCompressDispose(ic); ic = NULL;

	if (kFskErrNone != err)												/* If the specified format isn't accommodated, ... */
		err = FskBitmapEncodePNG(bm, &cmprImg, &cmprSize);				/* ... compress as PNG */
...
FskCanvas2dPathPolylineTo(FskCanvas2dContext ctx, FskCanvas2dPath path, UInt32 numPts, const double *pt)
{
	FskErr err	= kFskErrNone;
	if (!path) { if (!ctx) return kFskErrInvalidParameter; path = ctx->path; }
	if (kFskErrNone != FskGrowablePathGetLastPoint(path, NULL)) {													/* If there was no previous point, ... */
		if (((err = FskGrowablePathAppendSegmentFloatMoveTo(pt[0], pt[1], path)) != kFskErrNone) || --numPts == 0)	/* ... make an independent polyline */
			return err;
		pt += 2;
	}
	for ( ; numPts--; pt += 2)
