...
static void
sha1_process(struct sha1 *s, const UInt8 *blk)
{
	UInt32 W[16], a, b, c, d, e, w, T;
	int i;
...
void
sha1_update(struct sha1 *s, const void *data, UInt32 size)
{
	const UInt8 *p = data;
	UInt32 r = s->len % SHA1_BLKSIZE;

	s->len += size;
	if (r > 0) {
		UInt32 n = SHA1_BLKSIZE - r;
		if (size < n) {
...
void
sha1_fin(struct sha1 *s, UInt8 *dgst)
{
	UInt32 r = s->len % SHA1_BLKSIZE;
	UInt64 l;
	UInt8 *p;
	...
	s->buf[r++] = 0x80;
	if (r > SHA1_BLKSIZE - 8) {
		FskMemSet(s->buf + r, 0, SHA1_BLKSIZE - r);
		sha1_process(s, s->buf);
		r = 0;
	}
	FskMemSet(s->buf + r, 0, SHA1_BLKSIZE - 8 - r);
	l = s->len * 8;
...
static void
sha256_process(struct sha256 *s, const UInt8 *blk)
{
	UInt32 W[64], a, b, c, d, e, f, g, h, w, T1, T2;
	int i;
...
void
sha256_update(struct sha256 *s, const void *data, UInt32 size)
{
	const UInt8 *p = data;
	UInt32 r = s->len % SHA256_BLKSIZE;

	s->len += size;
	if (r > 0) {
		UInt32 n = SHA256_BLKSIZE - r;
		if (size < n) {
			FskMemCopy(s->buf + r, p, size);
			return;
		}
...
void
sha256_fin(struct sha256 *s, UInt8 *dgst)
{
	UInt32 r = s->len % SHA256_BLKSIZE;
	UInt64 l;
	UInt8 *p;
	...
	if (r > SHA256_BLKSIZE - 8) {
		FskMemSet(s->buf + r, 0, SHA256_BLKSIZE - r);
		sha256_process(s, s->buf);
		r = 0;
	}
	FskMemSet(s->buf + r, 0, SHA256_BLKSIZE - 8 - r);
	l = s->len * 8;
...
static void
sha512_process(struct sha512 *s, const UInt8 *blk)
{
	UInt64 W[80], a, b, c, d, e, f, g, h, w, T1, T2;
	int i;
...
void
sha512_update(struct sha512 *s, const void *data, UInt32 size)
{
	const UInt8 *p = data;
	UInt64 l = s->len[0];
	UInt32 r = l % SHA512_BLKSIZE;

	s->len[0] += size;
	if (s->len[0] < l)
		s->len[1]++;
	if (r > 0) {
		UInt32 n = SHA512_BLKSIZE - r;
		if (size < n) {
			FskMemCopy(s->buf + r, p, size);
			return;
		}
...
void
sha512_fin(struct sha512 *s, UInt8 *dgst)
{
	UInt32 r = s->len[0] % SHA512_BLKSIZE;
	UInt64 l1, l2;
	UInt8 *p;
	int i;
	...
	if (r > SHA512_BLKSIZE - 16) {
		FskMemSet(s->buf + r, 0, SHA512_BLKSIZE - r);
		sha512_process(s, s->buf);
		r = 0;
	}
	FskMemSet(s->buf + r, 0, SHA512_BLKSIZE - 16 - r);
