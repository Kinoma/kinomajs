...
#define bn_high_word(x)		(*(((bn_word *)&(x))+1))
#else
#define bn_high_word(x)		(*((bn_word *)&(x)))
#endif
#define bn_low_word(x)		((bn_word)(x))
#define bn_wordsize		(sizeof(bn_word) * 8)

void
bn_init_context(xsMachine *the, bn_context_t *ctx)
{
	ctx->the = the;
	ctx->bufptr = ctx->buf;
	ctx->bufend = ctx->buf + sizeof(ctx->buf);
}

static void
bn_throwError(bn_context_t *ctx, char *code)
{
	if (ctx->the != NULL) {
		xsMachine *the = ctx->the;
		cryptThrow(code);
	}
}

void
bn_freetbuf(bn_context_t *ctx, void *bufptr)
{
	ctx->bufptr = bufptr;
}

static int bn_max_alloc = 0;
...
bn_allocbuf(bn_context_t *ctx, unsigned int n)
{
	...
	if (ctx->bufptr >= ctx->bufend)
		bn_throwError(ctx, "kArithInsufficientTemporaryBuffer");
	if ((n = ctx->bufptr - ctx->buf) > bn_max_alloc)
		bn_max_alloc = n;
	return(p);
}
...
{
	bn_t *r;
	size_t sz = sizeof(bn_t) + ((n-1) * sizeof(bn_word));
...
bn_fill0(bn_t *r)
{
	FskMemSet(r->data, 0, r->size * sizeof(bn_word));
}

void
bn_copy(bn_t *a, bn_t *b)
{
	FskMemCopy(a->data, b->data, b->size * sizeof(bn_word));
	a->size = b->size;
	a->sign = b->sign;
}
...
bn_ffs(bn_t *a)
{
	...
	for (i = 0; i < bn_wordsize && !(w & ((bn_word)1 << (bn_wordsize - 1 - i))); i++)
		;
	return(i);
}
...
bn_lsl(bn_context_t *ctx, bn_t *r, bn_t *a, unsigned int sw)
{
	...
	bsz = sw % bn_wordsize;
	if (bsz == 0) {
		FskMemMove(&r->data[wsz], a->data, a->size * sizeof(bn_word));
		FskMemSet(r->data, 0, wsz * sizeof(bn_word));
		r->size = a->size + wsz;
	}
	else {
...
bn_lsr(bn_context_t *ctx, bn_t *r, bn_t *a, unsigned int sw)
{
	...
	if (r == NULL)
		r = bn_alloct(ctx, n);
	if (bsz == 0) {
		FskMemMove(r->data, &a->data[wsz], n * sizeof(bn_word));
		r->size = n;
	}
	else {
		for (i = 0; i < n - 1; i++)
...
bn_t *
bn_inc(bn_context_t *ctx, bn_t *r, bn_t *a, int d)
{
	bn_t *t;
...
mont_mul(bn_mod_t *mod, bn_t *r, bn_t *a, bn_t *b)
{
	...
	if (r == NULL)
		r = bn_alloct(mod->ctx, mod->m->size);
	n = mod->m->size;
	t1 = bn_alloct(mod->ctx, b->size + 1);
	t2 = bn_alloct(mod->ctx, mod->m->size + 1);
	t3 = bn_alloct(mod->ctx, MAX(t1->size, t2->size) + 1);
	t4 = bn_alloct(mod->ctx, t3->size + 1);
	t4->size = 1;
	t4->data[0] = 0;
	for (i = 0; i < n; i++) {
		bn_word u = (t4->data[0] + a->data[i] * b->data[0]) * mod->u;
		bn_umul1(mod->ctx, t1, b, a->data[i]);
		bn_umul1(mod->ctx, t2, mod->m, u);
		bn_uadd(mod->ctx, t3, t1, t2);
		bn_uadd(mod->ctx, t4, t4, t3);
		bn_lsr(mod->ctx, t4, t4, bn_wordsize);
	}
	if (bn_comp(t4, mod->m) >= 0)
		bn_sub(mod->ctx, t4, t4, mod->m);
	bn_copy(r, t4);
	bn_freetbuf(mod->ctx, t1);
	return(r);
}
...
mont_exp_init(bn_mod_t *mod, bn_t *r)
{
	...
	bn_fill0(t);
	t->data[t->size - 1] = 1;
	return(bn_mod(mod->ctx, r, t, mod->m));
}
...
bn_mod_mulinv(bn_mod_t *mod, bn_t *r, bn_t *a)
{
	...
	if (bn_iszero(a))
		bn_throwError(mod->ctx, "kArithDivideByZero");
	if (r == NULL)
		r = bn_alloct(mod->ctx, mod->m->size);
...
top:
	...
	if (u->size == 1 && u->data[0] == 0) {
		if (!(v->size == 1 && v->data[0] == 1))
			bn_throwError(mod->ctx, "kArithCalculationError");
		if (D->sign) {
			while (D->sign)
...
void
bn_mod_init(bn_mod_t *mod, bn_context_t *ctx, bn_t *m, unsigned int options)
{
	mod->ctx = ctx;
	mod->m = m;
	if (!(options & OPT_NOMONT) && m->data[0] & 1) {
		/* use Montgomery method */
	...
	mod->exp_func = (options & OPT_LR) ? bn_mod_exp_LR: bn_mod_exp_sliding_window;
